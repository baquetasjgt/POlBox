...
const ScreenEditProfile = ({ onBack }) => {
  ...
  const Field = ({ label, value, onChange, type = 'text', badge, disabled }) => (
    <div style={{ padding: '11px 16px', borderTop: `1px solid ${PB.line}` }}>
      <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase', color: PB.ink3, marginBottom: 4 }}>{label}</div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <input
          type={type} value={value} onChange={onChange} disabled={disabled}
          style={{
            flex: 1, border: 0, outline: 0, padding: 0, background: 'transparent',
            fontFamily: type === 'password' ? 'monospace' : PB.font,
            fontWeight: 600, fontSize: 15,
            color: disabled ? PB.ink3 : PB.ink,
...
Object.assign(window, { ScreenEditProfile });
