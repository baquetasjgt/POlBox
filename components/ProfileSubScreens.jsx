...
const OrderTicket = ({ order, onClose, fmt, total, tone }) => {
  ...
  const t = tone(o.state);
  return (
    <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(20,13,62,.5)', zIndex: 100, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', backdropFilter: 'blur(4px)' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: PB.surface, borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: '8px 20px 32px', maxHeight: '85%', overflowY: 'auto', boxShadow: '0 -20px 60px rgba(20,13,62,.2)' }}>
        <div style={{ width: 44, height: 4, borderRadius: 999, background: PB.line, margin: '8px auto 20px' }}/>

        {/* Cabecera */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
          <div>
            <div style={{ fontFamily: PB.mono, fontWeight: 700, fontSize: 13, color: PB.morado }}>{o.id}</div>
            <div style={{ fontFamily: PB.font, fontWeight: 800, fontSize: 20, letterSpacing: '-0.01em', marginTop: 2 }}>Detalle del pedido</div>
          </div>
          <span style={{ padding: '4px 10px', borderRadius: 999, background: t.bg, color: t.fg, fontSize: 11, fontWeight: 800 }}>{t.label}</span>
        </div>

        {/* Artículos */}
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase', color: PB.ink3, marginBottom: 8 }}>Artículos</div>
          {o.items.map((it, i) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderTop: `1px solid ${PB.line}`, fontSize: 13, color: PB.ink2 }}>
              <span>{it.name} ×{it.qty}</span>
              <span style={{ fontFamily: PB.mono, fontWeight: 600 }}>{fmt(it.price * it.qty)}</span>
            </div>
          ))}
          {o.shipping > 0 && (
            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderTop: `1px solid ${PB.line}`, fontSize: 13, color: PB.ink2 }}>
              <span>Envío</span><span style={{ fontFamily: PB.mono, fontWeight: 600 }}>{fmt(o.shipping)}</span>
            </div>
          )}
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderTop: `1px solid ${PB.line}` }}>
            <span style={{ fontFamily: PB.font, fontWeight: 800, fontSize: 15 }}>Total</span>
            <span style={{ fontFamily: PB.mono, fontWeight: 800, fontSize: 18 }}>{fmt(total(o))}</span>
          </div>
        </div>

        {/* Entrega */}
        <div style={{ padding: '14px', borderRadius: 16, background: o.delivery === 'locker' ? PB.mentaSoft : PB.warnBg, marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <Icon name={o.delivery === 'locker' ? 'bolt' : 'refresh'} size={16} color={PB.morado}/>
            <span style={{ fontFamily: PB.font, fontWeight: 700, fontSize: 14, color: PB.ink }}>
              {o.delivery === 'locker' ? `Locker ${o.lockerId} · POLEBOX ${o.venueName}` : `Envío a domicilio · ${o.addr}`}
            </span>
          </div>
          {o.delivery === 'locker' ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ fontSize: 12, color: PB.ink3 }}>Código de apertura:</span>
              <span style={{ fontFamily: PB.mono, fontWeight: 800, fontSize: 20, color: PB.morado, letterSpacing: '.12em' }}>{o.lockerCode}</span>
            </div>
          ) : (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ fontSize: 12, color: PB.ink3 }}>Seguimiento:</span>
              <span style={{ fontFamily: PB.mono, fontWeight: 700, fontSize: 13, color: PB.morado }}>{o.tracking}</span>
            </div>
          )}
        </div>

        {/* Pago */}
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 14px', borderRadius: 14, background: PB.surface2, marginBottom: 14, fontSize: 13 }}>
          <span style={{ color: PB.ink3 }}>Método de pago</span>
          <span style={{ fontFamily: PB.font, fontWeight: 700, color: PB.ink }}>{o.method}</span>
        </div>

        <button onClick={onClose} style={{ width: '100%', padding: '14px', borderRadius: 14, border: `1px solid ${PB.line}`, background: PB.surface, fontFamily: PB.font, fontWeight: 700, fontSize: 15, color: PB.ink, cursor: 'pointer' }}>
          Cerrar
        </button>
      </div>
    </div>
